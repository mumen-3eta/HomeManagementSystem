...
CREATE TABLE user_processor(
  ...
  user_id INTEGER REFERENCES users(id) NOT NULL ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE user_controller(
  ...
  processor_id INTEGER REFERENCES user_processor(id) NOT NULL ON UPDATE CASCADE ON DELETE CASCADE,
